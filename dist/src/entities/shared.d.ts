...
export interface ISharedDataConnects<T extends IModelEntity> {
    ...
    update: (query: IQueryOrPartial<T>, update: IModelEntityPartial<T>, config?: IModelConfigurationDetails) => Promise<T[]>;
    count: (query: IQueryOrPartial<T>, config?: IModelConfigurationDetails) => Promise<number>;
    destroy: (query: number | IModelEntityPartial<T>, config?: IModelConfigurationDetails) => Promise<T>;
    destroyAll: (query: IQueryOrPartial<T>, config?: IModelConfigurationDetails) => Promise<T[]>;
    create: (model: IModelEntityPartial<T>, config?: IModelConfigurationDetails) => Promise<T>;
    createMany: (query: IModelEntityPartial<T>[], config?: IModelConfigurationDetails) => Promise<T[]>;
    raw: (config?: IModelConfigurationDetails) => any;
    findOrCreate: (criteria: IQueryOrPartial<T>, initialsValues: IModelEntityPartial<T>, config?: IModelConfigurationDetails) => Promise<T>;
